...
INPUT_JSON = Path(str(row["json_path"]))
...
success_marker = AF3_ROOT / "TASK6_AF3_SUCCESS.txt"

if not success_marker.exists():
    raise FileNotFoundError(
        f"AF3 success marker not found: {success_marker}"
    )
...
OUTDIR = ROOT / "task6_runtime/bridge/pilot"
OUTDIR.mkdir(parents=True, exist_ok=True)

OUTPUT = OUTDIR / f"{INPUT_NAME}_bridge_meta.npz"
